...
const userIds = Array.from({ length: 1 }, generateId);
export const userId = userIds[0];
...
const initiateDb = async () => {
  try {
    userIds.map(async (id) => {
     ...
     const createdUser = await db.user.createMany({ data: users });

      const authentications = createAuthentications(id);
     const authUser = await db.authentication.createMany({ data: authentications });

      const accounts = createAccounts(id);
     const createdAccount = await db.account.createMany({ data: accounts });
      const accountIds = accounts.map((a) => a.id);

      const items = createItems(accountIds, id);
    const createdItems =  await db.item.createMany({ data: items });

      const objectives = createObjectives(accountIds, id);
   const createdObjectif =   await db.objectif.createMany({ data: objectives });
   return {
       users: createdUser,
       accounts : createdAccount,
         items: createdItems,
            objectifs: createdObjectif,
